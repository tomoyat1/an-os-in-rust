...
impl TaskList {
    ...
    pub fn new_task(&mut self) -> TaskHandle {
        ...
        let mut kernel_stack = unsafe {
            ...
            (*ptr).stack = [0; KERNEL_STACK_SIZE - size_of::<WithSpinLock<TaskInfo>>()];

            (*ptr).stack[(KERNEL_STACK_SIZE - size_of::<WithSpinLock<TaskInfo>>() - 8)
                ..(KERNEL_STACK_SIZE - size_of::<WithSpinLock<TaskInfo>>())]
    ...
    fn create_task(&mut self, mut kernel_stack: Box<Task>) -> usize {
        ...
        unsafe {
            kernel_stack.stack[(KERNEL_STACK_SIZE - size_of::<WithSpinLock<TaskInfo>>()) - 16
                ..(KERNEL_STACK_SIZE - size_of::<WithSpinLock<TaskInfo>>()) - 8]
                .copy_from_slice(&id.to_le_bytes());
        }
        kernel_stack.info.lock().registers.stack_top =
            &(kernel_stack.stack[(KERNEL_STACK_SIZE - size_of::<WithSpinLock<TaskInfo>>())
                - 8
                - size_of::<usize>() * 6]) as *const u8 as usize;
...
pub(crate) struct Task {
    ...
    stack: [u8; (KERNEL_STACK_SIZE - size_of::<WithSpinLock<TaskInfo>>())],
}
